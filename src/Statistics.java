...
//		Input input = Main.read("b_lovely_landscapes");
		Input input = Main.read("c_memorable_moments");
//		Input input = Main.read("d_pet_pictures");
//		Input input = Main.read("e_shiny_selfies");
	...
	public static void computeStats(Input input) {
		...
		System.out.println("Lower 25: " + tags.subList(0, 25).stream().map(t -> tagMap.get(t)).collect(Collectors.toList()));
		Collections.reverse(tags);
		System.out.println("Upper 25: " + tags.subList(0, 25).stream().map(t -> tagMap.get(t)).collect(Collectors.toList()));
	}
